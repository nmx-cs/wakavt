...
class Trainer(object):
    ...
    def _simple_forward(self, data_iter, mode="train"):
        assert mode in ("train", "valid", "test")
        tgt, tgt_lens, keywords, keyword_pos, *args = next(data_iter)
        loss_mask = tgt != self.opts.PAD_token
        logits, *vars = self.model(tgt, keywords, *args, mode=mode)
        return logits, vars, loss_mask
    ...
    def _forward(self, data_iter, mode="train"):
        ...
        if self.opts.is_cvae:
            ...
            expanded_tgt = self.model.expand_tgt(tgt, sample_n)
        real_tgt = self.model.get_real_tgt(expanded_tgt, keywords, sample_n)

        logits, *vars = self.model(tgt, keywords, *args, mode=mode)
        if self.opts.is_cvae:
            mu_p, log_var_p, mu_r, log_var_r, bow_logits = vars

        cur_ce = self.text_cross_entropy_loss(logits, real_tgt, reduction=self.opts.loss_reduction)
        cur_ppl = getPPL(logits, real_tgt, real_tgt != self.opts.PAD_token)
        cur_bow_loss = torch.zeros_like(cur_ce)
        if self.opts.is_cvae and self.opts.use_bow_loss:
            bow_tgt = self.model.get_bow_tgt(expanded_tgt)
            bow_loss_mask = loss_mask
            if sample_n > 1:
                bow_loss_mask = bow_loss_mask.repeat(1, sample_n)
            cur_bow_loss = self.bow_loss(bow_logits, bow_tgt, loss_mask=bow_loss_mask, reduction=self.opts.loss_reduction)
        cur_elbo = cur_kld = torch.zeros_like(cur_ce)
        kld_coeff = 0.
        if self.opts.is_cvae:
            ...
            cur_kld, kld_coeff = self.kld_loss(mu_r, log_var_r, kld_mode, loss_mask=loss_mask, prior_mu=mu_p,
                                               prior_log_var=log_var_p, reduction=self.opts.kld_loss_reduction)
            cur_elbo = cur_ce + cur_kld

        return {"cur_ce": cur_ce,
            ...
            if self.train_states["train_steps"] % self.opts.logging_rate == 0:
                ...
                print("Step {}  progress rate: {:.3f}  loss: {:.8f}  ppl: {:.6f}  "
                      "speed: {:.0f} samples per second  remaining (approximately): {}".format(
                          ...
                          self.train_states["cur_ppl"]["step"] / self.opts.logging_window,
    ...
    def _evaluate(self, data_iter, mode):
        ...
        records = {"loss": 0.0, "ce": 0.0, "kld": 0.0, "elbo": 0.0, "ppl": 0.0, "bow_loss": 0.0}
        ...
        with torch.no_grad():
            for _ in range(num_batch):
                ...
                var_dict["cur_ce"] = cur_ce = var_dict["cur_ce"] * cur_bsz
                var_dict["cur_kld"] = cur_kld = var_dict["cur_kld"] * cur_bsz
                var_dict["cur_elbo"] = var_dict["cur_ce"] + var_dict["cur_kld"]
                var_dict["cur_bow_loss"] = cur_bow_loss = var_dict["cur_bow_loss"] * cur_bsz
                var_dict["cur_ppl"] *= cur_bsz
                cur_loss = cur_ce * self.opts.ce_coeff + cur_kld + cur_bow_loss * self.opts.bow_coeff
                var_dict["cur_loss"] = cur_loss
        ...
        if mode == "valid":
            print(
                ...
                (records["loss"] / self.opts.valid_size,
                 ...
                 records["ppl"] / self.opts.valid_size,
                 self.train_states["cur_ppl"]["mean"] / num_steps_since_last,
            ...
            print("Test:  test loss: %.8f  test ppl: %.6f speed: %.0f samples per second" %
                  (records["loss"] / self.opts.test_size,
                   records["ppl"] / self.opts.test_size,
