...
class Embeddings(nn.Module):
    def __init__(self,
        ...
        if use_pos_emb:
            self.pos_emb = PositionEmbedding(
                pos_emb_type,
                pretrained_embs.get("pos", None),
                embedding_dim=embedding_dim["pos"],
                output_dim=output_dim,
                max_seq_len=max_seq_len,
                PAD_token=PAD_token,
                finetune=embs_finetune_option["pos"],
                align_pos=pos_emb_align_pos,
                initialization_method=initialization_method["pos"])
    ...
    def forward(self,
        ...
        segment_align_pos, positional_align_pos = align_pos
        all_embs = {}
        ...
        if use_pos_encodings and self.use_pos_emb:
            all_embs["pos"] = self.pos_emb(word_ids, align_pos=positional_align_pos)
        if use_segment_encodings and self.use_segment_emb:
            all_embs["segment"] = self.segment_emb(
                word_ids,
                align_pos=segment_align_pos,
        ...
        out = emb
        if "pos" in all_embs:
            if self.pos_emb_type == "trainable":
                out = self.dropout(self.layer_norm(out + all_embs["pos"]))
            else:
                out = self.dropout(self.layer_norm(out)) + all_embs["pos"]
    ...
    def get_unexpanded_pos_encodings(self, seq_len, align_pos=0):
        assert hasattr(self, "pos_emb")
        return self.pos_emb.get_unexpanded_encodings(seq_len, align_pos)
...
class PositionEmbedding(EmbeddingBase):
    def __init__(self,
                 type,
                 pretrained_weights,
                 *,
                 ...
                 align_pos,
                 initialization_method):
        super(PositionEmbedding, self).__init__(embedding_dim, output_dim)

        self.type = type.lower()
        assert self.type in ("sinusoidal", "trainable")
        assert self.type == "trainable" or embedding_dim == output_dim
        assert embedding_dim % 2 == 0
        self.PAD_token = PAD_token
        self.align_pos = align_pos % max_seq_len
        max_seq_len += self.align_pos
        self.max_seq_len = max_seq_len

        if self.type == "sinusoidal":
            positions = torch.arange(0, max_seq_len, dtype=torch.get_default_dtype()).unsqueeze(-1)
            div_term = torch.arange(0, embedding_dim, 2, dtype=torch.get_default_dtype())
            div_term = torch.exp(div_term * (- math.log(10000.0)) / embedding_dim).unsqueeze(0)
            pos_embedding = torch.zeros(max_seq_len, embedding_dim, dtype=torch.get_default_dtype())
            ...
            if pretrained_weights is not None:
                pretrained_weights = torch.as_tensor(pretrained_weights, dtype=torch.get_default_dtype())
                pretrained_weights = pretrained_weights[:max_seq_len]
                assert tuple(pretrained_weights.shape) == (max_seq_len, embedding_dim)
                self.pos_embedding = nn.Parameter(pretrained_weights, requires_grad=finetune)
            else:
                self.pos_embedding = nn.Parameter(torch.Tensor(max_seq_len, embedding_dim), requires_grad=True)
                initialize_embedding(self.pos_embedding, initialization_method)

    # input : [seq_len, *]
    def forward(self, input, align_pos=0):
        encoded = self.get_unexpanded_encodings(input.size(0), align_pos)
        extra_dims = [1] * (input.dim() - 1)
        encoded = encoded.view(input.size(0), *extra_dims, -1).expand(-1, *input.shape[1:], -1)
        return encoded

    def get_unexpanded_encodings(self, seq_len, align_pos):
        assert -seq_len <= align_pos < seq_len
        start_ind = self.align_pos - align_pos
        assert start_ind >= 0
        encoded = self.pos_embedding[start_ind:(start_ind + seq_len)]
        assert encoded.size(0) == seq_len
        return self.out_proj(encoded.clone())
